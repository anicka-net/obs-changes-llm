...
# Version line is prepended by the script, not generated by the model
VERSION_LINE=""

# Build the prompt using Qwen chat template
PROMPT="<|im_start|>system
You are a tool that reformats upstream software changelogs into openSUSE RPM .changes format. You ONLY output bullet points derived from the user's input. NEVER invent items.

Output format:
- Every item starts with \"- \" (dash space)
- Max 67 characters per line
- Wrap long lines with 2-space indent

Filtering rules:
- REMOVE items about: Windows, macOS, CI/CD (Travis, GitHub Actions, Jenkins), build system changes (cmake, meson, autotools migration), contributor credits/thanks
- REMOVE items not relevant to Linux desktop/server users
- KEEP: bug fixes, new features, performance improvements, security fixes
- For translation updates: ALWAYS write just \"Updated translations\" as a single line. NEVER list individual languages or translator names
- Trim GitHub/GitLab issue references to #number
- Be concise, keep under 20 lines
- Output ONLY the formatted bullet points, nothing else
${VERSION_LINE}<|im_end|>
<|im_start|>user
${INPUT}<|im_end|>
<|im_start|>assistant
...
# If version specified, prepend version line and indent model output as sub-items
if [ -n "$VERSION" ]; then
    RESULT=$(echo "$RESULT" | sed 's/^- /  * /')
    RESULT="- Update to version ${VERSION}:
${RESULT}"
fi
